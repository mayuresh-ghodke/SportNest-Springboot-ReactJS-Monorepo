...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "select " +
            "p.product_id, p.name, p.description, p.current_quantity, p.cost_price, p.category_id, p.sale_price, p.image, p.is_activated, p.is_deleted, p.subcategory_id " +
    ...
    @Query(value = "select p.product_id, p.name, p.description, p.current_quantity, p.cost_price, p.category_id, p.sale_price, p.image, p.is_activated, p.is_deleted, p.subcategory_id from products p where p.is_deleted = false and p.is_activated = true limit 4", nativeQuery = true)
    ...
    // need to be 
    //@Query(value = "select p from Product p inner join SubCategory sc on sc.id = ?1 and p.subcategory.id = ?1 where p.is_activated = true and p.is_deleted = false")
    List<Product> findProductsBySubCategoryId(Long id);

    @Modifying
    @Query("""
    UPDATE Product p
    SET p.currentQuantity = p.currentQuantity - :qty
    WHERE p.id = :id AND p.currentQuantity >= :qty""")
